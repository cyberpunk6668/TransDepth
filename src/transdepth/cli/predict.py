...
from transdepth.inference.merge import build_exported_predictor
...
def main() -> None:
    ...
    is_export = raw_checkpoint.get("schema_version") == "td_rgb_export_v1"
    if not is_export:
    ...
    if is_export:
        model = build_exported_predictor(config, checkpoint_path).to(device)
    else:
    ...
    for index in range(limit):
        ...
        with torch.no_grad(), torch.autocast("cuda", dtype=torch.bfloat16):
            inputs = sample.rgb[None].to(device)
            prediction = model(inputs) if is_export else model(inputs, return_aux=False).depth_m
